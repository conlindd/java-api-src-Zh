 ...
 *
 * <p>
 *  输入法事件包含关于使用输入法构成的文本的信息。每当文本更改时,输入法将发送一个事件。如果当前正在使用输入法的文本组件是活动客户端,则会将事件分派给该组件。否则,它将被分派到单独的合成窗口。
 * 
 * <p>
 *  输入法事件包含的文本由两部分组成：提交文本和组合文本。任一部分可以为空。这两个部分一起替换在先前事件中发送的任何未提交的组合文本,或当前选择的提交文本。
 * 提交的文本应该被集成到文本组件的持久数据中,不会再被发送。组合文本可以重复发送,其中包含更改以反映用户的编辑操作。提交的文本始终位于组合文本之前。
 * 
 * 
...
public class InputMethodEvent extends AWTEvent {
     ...
     * <p>
     *  事件类型指示更改的输入法文本。在处理输入时,此事件由输入法生成。
     * 
     */
     ...
     * <p>
     *  事件类型指示输入法文本中的更改的插入点。如果只更改插入符号,则在处理输入时由输入方法生成此事件。
     * 
     */
     ...
     *
     * <p>
     * 使用指定的源组件,类型,时间,文本,插入符和visiblePosition构造一个<code> InputMethodEvent </code>。
     * <p>
     *  插入符和visiblePosition的偏移量相对于当前组合文本;即<code> text </code>中的组合文本,如果这是<code> INPUT_METHOD_TEXT_CHANGED </code>
     * 事件,则前面的<code> INPUT_METHOD_TEXT_CHANGED </code> / code>事件。
     ...
     *
     * <p>
     *  使用指定的源组件,类型,文本,插入符号和visiblePosition构造一个<code> InputMethodEvent </code>。
     * <p>
     *  插入符和visiblePosition的偏移量相对于当前组合文本;即<code> text </code>中的组合文本,如果这是<code> INPUT_METHOD_TEXT_CHANGED </code>
     * 事件,则前面的<code> INPUT_METHOD_TEXT_CHANGED </code> / code>事件。
     * 此事件的时间戳通过调用{@link java.awt.EventQueue#getMostRecentEventTime()}初始化。
     *  <p>请注意,传入无效的<code> id </code>会导致未指定的行为。
     * 如果<code> source </code>是<code> null </code>,此方法会抛出<code> IllegalArgumentException </code>。
     * 
     * 
     ...
     *
     * <p>
     * 使用指定的源组件,类型,插入符号和visiblePosition构造一个<code> InputMethodEvent </code>。
     * 文本设置为<code> null </code>,<code> committedCharacterCount </code>为0。
     * <p>
     *  <code> caret </code>和<code> visiblePosition </code>的偏移量相对于当前编写的文本;即如果事件被构造为<code> CARET_POSITION_CHA
     * NGED </code>事件,则在前面的<code> INPUT_METHOD_TEXT_CHANGED </code>事件的<code> text </code>对于没有文本的<code> INPUT
     * _METHOD_TEXT_CHANGED </code>事件,<code> caret </code>和<code> visiblePosition </code>必须是<code> null </code>
     * 。
     * 此事件的时间戳通过调用{@link java.awt.EventQueue#getMostRecentEventTime()}初始化。
     *  <p>请注意,传入无效的<code> id </code>会导致未指定的行为。
     * 如果<code> source </code>是<code> null </code>,此方法会抛出<code> IllegalArgumentException </code>。
     ...
     *
     * <p>
     *  获取组合的提交和组合文本。从索引0到索引</em>的字符getCommittedCharacterCount() -  1 </code>是提交的文本,其余的字符组成文本。
     * 
     * 
     * @return the text.
     ...
     *  获取插入符号。
     * <p>
     *  插入符的偏移量相对于当前组合文本;即getText()中的组合文本,如果这是一个<code> INPUT_METHOD_TEXT_CHANGED </code>事件,则在前面的<code> INPUT
     * _METHOD_TEXT_CHANGED </code>事件的getText()。
     * 
     * 
     ...
     * 获得最重要的位置是可见的。
     * <p>
     *  可见位置的偏移相对于当前组成文本;即getText()中的组合文本,如果这是一个<code> INPUT_METHOD_TEXT_CHANGED </code>事件,则在前面的<code> INPUT
     * _METHOD_TEXT_CHANGED </code>事件的getText()。
     * 
     * 
     ...
     *
     * <p>
     *  返回标识此事件的参数字符串。此方法对于事件日志记录和调试非常有用。它包含文本形式的事件ID,用"+"分隔的提交和组合文本的字符,提交字符数,插入符号和可见位置。
     * 
     * 
     ...
     * <p>
     *  如果</code>字段(如果对象输入流中不存在),则初始化<code>。
     * 在这种情况下,将通过调用{@link java.awt.EventQueue#getMostRecentEventTime()}来初始化该字段。
     * 
     */
