 ...
 *  <P>它通过扩展NotificationBroadcasterSupport来实现NotificationBroadcaster,以便在删除关系时发送通知。
 *  <P>它实现了NotificationListener接口,以便能够接收关于在关系角色和关系MBean中引用的MBean的注销的通知。
 *  <P>它实现MBeanRegistration接口,以便能够检索其ObjectName和MBean Server。
 * 
 * 
 * @since 1.5
 */
public class RelationService extends NotificationBroadcasterSupport
     ...
     *
     * <p>
     *  返回标志以指示是否接收到关于在关系中引用的MBean的注销的通知,如果必须执行关系的立即"清除"(查找关系不再有效),或者如果将要执行仅在将明确调用purgeRelations方法时执行。
     *  <P> true是立即清除。
     * 
     * 
     ...
     *
     * <p>
     * 设置标志以指示是否接收到关于在关系中引用的MBean的注销的通知,如果必须执行关系的立即"清除"(寻找关系不再有效),或者如果将要执行仅在将明确调用purgeRelations方法时执行。
     *  <P> true是立即清除。
     * 
     * 
     ...
     *   - 具有唯一的关系ID未在当前关系服务<P>中 - 具有关系类型在关系服务<P>中创建的关系类型 - 具有符合关系类型中提供的角色信息的角色。
     * 添加由用户创建的MBean(并由他在MBean服务器中注册)作为关系服务中的关系。
     * 
     * 
     ...
     *
     * <p>
     *  在发生JMX通知时调用。目前处理MBeans注销的通知,在关系角色中引用或作为关系本身。
     * 
     * 
     ...
     * <p>
     *  返回一个NotificationInfo对象,其中包含通知的Java类的名称和发送的通知类型。
     */
