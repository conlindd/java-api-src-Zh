 ...
 *
 * <p>
 *  此类用于从代理向管理器发送INFORM REQUESTS。
 * 
 *  创建,控制和管理一个或多个通知请求。
 * 
 *  SnmpSession维护所有活动通知请求的列表并通知响应。
 * 每个SnmpSession都有一个dispatcher,它是一个线程,用于服务它创建的所有通知请求,每个SnmpSession使用一个单独的套接字发送/接收通知请求/响应。
 * 
 *  SnmpSession对象与SNMP适配器服务器相关联。它是在SNMP适配器服务器第一次发送通知请求时创建的,并在SNMP适配器服务器停止时被销毁(及其关联的SnmpSocket)。
 * 
 */
...
class SnmpSession implements SnmpDefinitions, Runnable {
     ...
     * <p>
     *  SnmpSocket用于通过所有通知在此会话中创建的请求与管理器通信。
     * 
     */
     ...
     * <p>
     *  将为所有通知响应的分派器通知使用此会话对象生成的请求。 SnmpSession对象创建一个或多个通知请求。因此,当通知响应到达由会话产生的通知请求时,它服务由该会话对象创建的所有通知请求。
     * 
     */
     ...
     * <p>
     * 请求从会话线程同步。当用户从回调执行同步操作时,会发生这种情况。
     * 
     */
     ...
     * <p>
     *  指示此会话的线程是否处于活动状态,并且SNMP适配器服务器处于ONLINE状态。
     * 
     * 
     ...
     * <p>
     *  获取将在此会话中创建的通知请求使用的SnmpQManager。
     * 
     * 
     ...
     * <p>
     *  如果当前正在执行的线程是此会话的调度程序,则返回<CODE> true </CODE>。通常用于检测用户是否正在从此调度器上下文执行同步操作。例如,用户使用其关联的会话从请求回调中给出同步命令。
     * 
     * 
     ...
     * <p>
     *  使用指定的团体字符串向指定的InetAddress目标发送通知请求。
     * 
     * 
     ...
     * <p>
     *  对活动请求执行同步操作。任何数量的通知请求都可以在同步模式下完成,但每个线程只能有一个通知请求。用户只能使用请求句柄进行同步操作。
     * 
     */
     ...
     * <p>
     *  这个会话线程的Dispatcher方法。这是dispatcher方法,它进入无限循环并等待服务通知请求,它接收到来自管理器的答复。
     * 
     */
    @Override
    public void run() {
     ...
     * <p>
     * 将接收到对会话生成的通知请求的响应的通知请求对象添加。这被添加到私人商店,其将最终被调度器拾取用于处理。
     * 
     * 
     ...
     * <p>
     *  销毁任何待处理的通知请求,然后停止会话。此方法返回后,会话将无法使用。
     * 
     */
     ...
     * <p>
     *  <CODE> SnmpSession </CODE>对象的终结器。当垃圾回收确定没有对对象的更多引用时,垃圾收集器在对象上调用此方法。
     *  <P>删除此SNMP会话的所有请求,关闭套接字并将对<CODE> SnmpSession </CODE>对象的所有引用设置为<CODE> null </CODE>。
     */
    @Override
