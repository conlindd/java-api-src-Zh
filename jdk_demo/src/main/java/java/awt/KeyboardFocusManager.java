 ...
 *
 * <p>
 *  KeyboardFocusManager负责管理活动和焦点的Windows以及当前焦点所有者。焦点所有者被定义为应用程序中的组件,通常会接收用户生成的所有KeyEvent。
 * 关注的窗口是焦点所有者的或包含焦点所有者的窗口。只有框架或对话框可以是活动窗口。本地加窗系统可以表示活动窗口或其具有特殊装饰的子项,例如突出显示的标题栏。
 * 活动窗口始终是关注的窗口,或者是焦点窗口的所有者的第一个框架或对话框。
 * <p>
 *  KeyboardFocusManager既是客户端代码查询焦点所有者并启动焦点更改的集中位置,也是所有FocusEvents,与焦点相关的WindowEvent和KeyEvents的事件分派器。
 * <p>
 * 一些浏览器将不同代码库中的applet分割成单独的上下文,并在这些上下文之间建立壁。在这种情况下,每个上下文将有一个KeyboardFocusManager。
 * 其他浏览器将所有applet放在同一个上下文中,这意味着对于所有applet只有一个单独的全局KeyboardFocusManager。此行为是实现相关的。有关详细信息,请参阅浏览器的文档。
 * 然而,无论有多少上下文,每个ClassLoader永远不能有多个焦点所有者,聚焦窗口或活动窗口。
 * <p>
 *  请参见
 * <a href="https://docs.oracle.com/javase/tutorial/uiswing/misc/focus.html">
    ...
    implements KeyEventDispatcher, KeyEventPostProcessor
{
     ...
     * 如果指定了null,那么将使用DefaultKeyboardFocusManager的新实例替换当前KeyboardFocusManager。
     * <p>
     * 如果安装了SecurityManager,则必须授予调用线程AWTPermission"replaceKeyboardFocusManager",以替换当前的KeyboardFocusManager。
     * 如果未授予此权限,此方法将抛出SecurityException,并且当前KeyboardFocusManager将保持不变。
     * 
     * 
     ...
     * <p>
     *  应用程序中将在未完成的临时焦点传输完成时重新获得焦点的组件,或焦点所有者(如果没有未完成的临时传输)。
     * 
     */
     ...
     * Window.
     * <p>
     *  只有框架或对话框可以是活动窗口。本地加窗系统可以用特殊装饰(诸如突出显示的标题栏)来表示活动窗口。活动窗口始终是关注的窗口,或者是焦点窗口的所有者的第一个框架或对话框。
     * 
     */
     ...
     * children).
     * <p>
     *  所有Windows的默认FocusTraversalPolicy没有自己的策略集。
     * 如果这些Windows有焦点循环根孩子没有自己的键盘遍历策略,那么这些孩子也将继承此策略(将递归地,它们的焦点循环根孩子)。
     * 
     */
     ...
     * set.
     * <p>
     * 默认焦点遍历键。每个遍历键数组将在所有没有自己显式设置的数组的Windows上生效。每个数组也将被那些没有自己明确设置的数组的Windows的任何子组件递归继承。
     * 
     */
     ...
     * possibilities.
     * <p>
     *  当前焦点周期根。如果焦点拥有者本身是焦点循环根,则关于哪些组件表示在正常焦点遍历期间要聚焦的下一个和前一个组件可能是不明确的。在这种情况下,当前焦点循环根用于区分各种可能性。
     * 
     */
     ...
     * <p>
     *  已注册的任何VetoableChangeListeners的描述。
     * 
     */
     ...
     * <p>
     *  已注册的任何PropertyChangeListeners的描述。
     * 
     */
     ...
     * a List of length 0.
     * <p>
     *  这个KeyboardFocusManager的KeyEventDispatcher链。
     * 该列表不包括此KeyboardFocusManager,除非它通过调用<code> addKeyEventDispatcher </code>显式重新注册。
     * 如果没有注册其他KeyEventDispatchers,此字段可以为null或引用长度为0的列表。
     * 
     */
     ...
     * <p>
     *  这个KeyboardFocusManager的KeyEventPostProcessor链。
     * 该列表不包括此KeyboardFocusManager,除非它通过调用<code> addKeyEventPostProcessor </code>显式重新注册。
     * 如果没有注册其他KeyEventPostProcessor,则此字段可以为空或引用长度为0的列表。
     * 
     */
     ...
     * <p>
     *  SequencedEvent,当前在AppContext中分派。
     * 
     */
     ...
     *
     * <p>
     *  如果焦点所有者与调用线程位于相同的上下文中,则返回焦点所有者。焦点所有者被定义为应用程序中的组件,通常会接收用户生成的所有KeyEvent。
     * 如果为焦点所有者启用焦点遍历键,则不会传递映射到焦点所有者焦点遍历键的KeyEvent。此外,KeyEventDispatchers可能在它们到达焦点所有者之前重新定位或使用KeyEvent。
     * 
     * 
     ...
     *
     * <p>
     *  返回焦点所有者,即使调用线程处于与焦点所有者不同的上下文中。焦点所有者被定义为应用程序中的组件,通常会接收用户生成的所有KeyEvent。
     * 如果为焦点所有者启用焦点遍历键,则不会传递映射到焦点所有者焦点遍历键的KeyEvent。此外,KeyEventDispatchers可能在它们到达焦点所有者之前重新定位或使用KeyEvent。
     * <p>
     *  如果这个KeyboardFocusManager不是当前调用线程上下文的KeyboardFocusManager,这个方法将抛出一个SecurityException异常。
     * 
     * 
     ...
     *
     * <p>
     * 设置焦点所有者。如果组件不可对焦,则操作将被取消。焦点所有者被定义为应用程序中的组件,通常会接收用户生成的所有KeyEvent。
     * 如果为焦点所有者启用焦点遍历键,则不会传递映射到焦点所有者焦点遍历键的KeyEvent。此外,KeyEventDispatchers可能在它们到达焦点所有者之前重新定位或使用KeyEvent。
     * <p>
     *  此方法实际上不将焦点设置为指定的组件。它仅存储随后由<code> getFocusOwner()</code>返回的值。
     * 使用<code> Component.requestFocus()</code>或<code> Component.requestFocusInWindow()</code>更改焦点所有者,但受平台限制
     * 。
     *  此方法实际上不将焦点设置为指定的组件。它仅存储随后由<code> getFocusOwner()</code>返回的值。
     * 
     * 
     ...
     *
     * <p>
     *  如果焦点所有者存在并且驻留在与调用线程相同的上下文中,则清除Java和本机级别的焦点所有者,否则该方法将静默返回。
     * <p>
     *  焦点所有者组件将接收到一个永久的FOCUS_LOST事件。
     * 此操作完成后,本地窗口系统将丢弃所有用户生成的KeyEvent,直到用户选择一个新的组件接收焦点,或者一个组件通过调用{@code requestFocus()}显式地给予焦点。
     ...
     *
     * <p>
     * 在Java和本机级别清除全局焦点所有者。如果存在焦点所有者,则该组件将接收到一个永久FOCUS_LOST事件。
     * 此操作完成后,本地窗口系统将丢弃所有用户生成的KeyEvent,直到用户选择一个新的组件来接收焦点,或者一个组件通过调用<code> requestFocus()</code>明确地给出焦点。
     * 此操作不会更改聚焦或活动的Windows。
     * <p>
     *  如果安装了SecurityManager,则调用线程必须授予"replaceKeyboardFocusManager"AWTPermission。
     * 如果未授予此权限,则此方法将抛出SecurityException,并且不会清除当前焦点所有者。
     * <p>
     *  此方法旨在仅由KeyboardFocusManager设置为当前KeyboardFocusManager用于调用线程的上下文。它不是一般客户使用。
     * 
     * 
     ...
     *
     * <p>
     *  如果永久焦点所有者与调用线程处于相同的上下文中,则返回永久焦点所有者。永久焦点所有者被定义为应用程序中接收永久FOCUS_GAINED事件的最后一个组件。
     * 除非临时焦点更改当前有效,否则焦点所有者和永久焦点所有者是等效的。在这种情况下,当临时焦点改变结束时,永久焦点拥有者将再次成为焦点拥有者。
     * 
     * 
     ...
     *
     * <p>
     * 返回永久焦点所有者,即使调用线程处于与永久焦点所有者不同的上下文中。永久焦点所有者被定义为应用程序中接收永久FOCUS_GAINED事件的最后一个组件。
     * 除非临时焦点更改当前有效,否则焦点所有者和永久焦点所有者是等效的。在这种情况下,当临时焦点改变结束时,永久焦点拥有者将再次成为焦点拥有者。
     * 
     * 
     ...
     *
     * <p>
     *  设置永久焦点所有者。如果组件不可对焦,则操作将被取消。永久焦点所有者被定义为应用程序中接收永久FOCUS_GAINED事件的最后一个组件。
     * 除非临时焦点更改当前有效,否则焦点所有者和永久焦点所有者是等效的。在这种情况下,当临时焦点改变结束时,永久焦点拥有者将再次成为焦点拥有者。
     * <p>
     *  此方法实际上不将焦点设置为指定的组件。它仅存储随后由<code> getPermanentFocusOwner()</code>返回的值。
     * 使用<code> Component.requestFocus()</code>或<code> Component.requestFocusInWindow()</code>更改焦点所有者,但受平台限制
     * 。
     *  此方法实际上不将焦点设置为指定的组件。它仅存储随后由<code> getPermanentFocusOwner()</code>返回的值。
     * 
     ...
     *
     * <p>
     *  如果关注的窗口与调用线程处于相同的上下文中,则返回关注的窗口。聚焦窗口是包含焦点所有者的窗口。
     * 
     * 
     ...
     *
     * <p>
     * 返回关注的窗口,即使调用线程位于与所关注的窗口不同的上下文中。聚焦窗口是包含焦点所有者的窗口。
     * 
     * 
     ...
     *
     * <p>
     *  设置聚焦窗口。聚焦窗口是包含焦点所有者的窗口。如果指定的对焦窗口不是可对焦的窗口,则操作将被取消。
     * <p>
     *  这种方法实际上不改变聚焦的窗口,就本地窗口系统而言。它只是存储由<code> getFocusedWindow()</code>后续返回的值。
     * 使用<code> Component.requestFocus()</code>或<code> Component.requestFocusInWindow()</code>更改聚焦窗口,但受平台限制。
     *  这种方法实际上不改变聚焦的窗口,就本地窗口系统而言。它只是存储由<code> getFocusedWindow()</code>后续返回的值。
     * 
     * 
     ...
     *
     * <p>
     *  如果活动窗口与调用线程位于相同的上下文中,则返回活动窗口。只有框架或对话框可以是活动窗口。本地加窗系统可以表示活动窗口或其具有特殊装饰的子项,例如突出显示的标题栏。
     * 活动窗口始终是关注的窗口,或者是焦点窗口的所有者的第一个框架或对话框。
     * 
     * 
     ...
     *
     * <p>
     * 返回活动窗口,即使调用线程处于与活动窗口不同的上下文中。只有框架或对话框可以是活动窗口。本地加窗系统可以表示活动窗口或其具有特殊装饰的子项,例如突出显示的标题栏。
     * 活动窗口始终是关注的窗口,或者是焦点窗口的所有者的第一个框架或对话框。
     * 
     * 
     ...
     *
     * <p>
     *  设置活动窗口。只有框架或对话框可以是活动窗口。本地加窗系统可以表示活动窗口或其具有特殊装饰的子项,例如突出显示的标题栏。活动窗口始终是关注的窗口,或者是焦点窗口的所有者的第一个框架或对话框。
     * <p>
     *  就本地窗口系统而言,此方法实际上不改变活动窗口。它只存储由<code> getActiveWindow()</code>后续返回的值。
     * 使用<code> Component.requestFocus()</code>或<code> Component.requestFocusInWindow()</code>更改活动窗口,但受平台限制。
     *  就本地窗口系统而言,此方法实际上不改变活动窗口。它只存储由<code> getActiveWindow()</code>后续返回的值。
     * 
     * 
     ...
     *
     * <p>
     *  返回默认的FocusTraversalPolicy。顶级组件在创建时使用此值,通过显式调用Container.setFocusTraversalPolicy来初始化自己的焦点遍历策略。
     * 
     * 
     ...
     *
     * <p>
     * 设置默认的FocusTraversalPolicy。顶级组件在创建时使用此值,通过显式调用Container.setFocusTraversalPolicy来初始化自己的焦点遍历策略。
     * 注意：此调用不会影响已创建的组件,因为它们的策略已初始化。只有新组件将使用此策略作为其默认策略。
     * 
     * 
     ...
     *
     * <p>
     *  设置给定遍历操作的默认焦点遍历键。此遍历键{@code Set}将在所有没有自己明确定义的{@code Set}的{@code Window}上生效。
     * 此{@code Set}也将由{@code Windows}中没有自己明确定义的{@code Set}的任何子级{@code Component}以递归方式继承。
     * <p>
     *  默认焦点遍历键的默认值是与实现相关的。 Sun建议特定本机平台的所有实现使用相同的默认值。下面列出了对Windows和Unix的建议。这些建议在Sun AWT实现中使用。
     * 
     * <table border=1 summary="Recommended default values for focus traversal keys">
     * <tr>
     ...
     * </tr>
     * <tr>
     *  <td> {@ code KEY_PRESSED},{@code CTRL-TAB}在{@ code KEY_PRESSED}上的{@ code TAB} {@ code KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS}
     *  </td> <td>正常向前键盘遍历</td>代码KEY_PRESSED} </td>。
     * </tr>
     * <tr>
     *  <td> {@ code KEY_PRESSED},{@code CTRL-SHIFT-TAB}上的{@ code SHIFT-TAB} {@ code KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS}
     *  </td> <td>正常反向键盘遍历</td> <td> }上{@code KEY_PRESSED} </td>。
     * </tr>
     * <tr>
     * <td> {@ keyboardFocusFocusManager.UP_CYCLE_TRAVERSAL_KEYS} </td> <td>向上移动一个焦点遍历周期</td> <td> none </td>
     * 。
     * </tr>
     * <tr>
     *  <td> {@ keyboardFocusManager.DOWN_CYCLE_TRAVERSAL_KEYS} </td> <td>向下一个焦点遍历周期</td> <td>无</td>
     * </tr>
     * </table>
     * 
     *  要禁用遍历键,请使用空的{@code Set};建议使用{@code Collections.EMPTY_SET}。
     * <p>
     *  使用{@code AWTKeyStroke} API,客户端代码可以指定两个特定的{@code KeyEvent} {@code KEY_PRESSED}或{@code KEY_RELEASED}中的
     * 哪一个,焦点遍历操作将发生。
     * 不过,无论指定哪个{@code KeyEvent},所有与焦点遍历键相关的{@code KeyEvent}(包括关联的{@code KEY_TYPED})事件都将被使用,并且不会被分派到任何{@代码Component}
     * 。
     * 将{@code KEY_TYPED}事件指定为映射到焦点遍历操作,或将同一个事件映射到多个默认焦点遍历操作,这是一个运行时错误。
     * <p>
     ...
     *
     * <p>
     *  返回给定遍历操作的一组默认焦点遍历键。此遍历键集将在没有这样的Set的自己明确定义的所有Windows上生效。这个Set也将被那些Windows的任何子组件递归继承,没有这样的Set自己明确定义。
     *  (有关每个操作的完整描述,请参阅<code> setDefaultFocusTraversalKeys </code>。)。
     * 
     * 
     ...
     *
     * <p>
     * 如果当前焦点循环根与调用线程位于相同的上下文中,则返回当前焦点循环根。如果焦点拥有者本身是焦点循环根,则关于哪些组件表示在正常焦点遍历期间要聚焦的下一个和前一个组件可能是不明确的。
     * 在这种情况下,当前焦点循环根用于区分各种可能性。
     * <p>
     *  此方法旨在仅由KeyboardFocusManagers和焦点实现使用。它不是一般客户使用。
     * 
     * 
     ...
     *
     * <p>
     *  返回当前焦点循环根,即使调用线程位于与当前焦点循环根不同的上下文中。如果焦点拥有者本身是焦点循环根,则关于哪些组件表示在正常焦点遍历期间要聚焦的下一个和前一个组件可能是不明确的。
     * 在这种情况下,当前焦点循环根用于区分各种可能性。
     * 
     * 
     * @return the current focus cycle root, or null if the current focus cycle
     ...
     *
     * <p>
     *  设置当前焦点循环根。如果焦点拥有者本身是焦点循环根,则关于哪些组件表示在正常焦点遍历期间要聚焦的下一个和前一个组件可能是不明确的。在这种情况下,当前焦点循环根用于区分各种可能性。
     * <p>
     *  如果安装了SecurityManager,则调用线程必须授予"replaceKeyboardFocusManager"AWTPermission。
     * 如果未授予此权限,则此方法将抛出SecurityException,并且当前焦点循环根将不会更改。
     * <p>
     * 此方法旨在仅由KeyboardFocusManagers和焦点实现使用。它不是一般客户使用。
     * 
     * 
     ...
     * <ul>
     *  </li> <li>焦点拥有者("focusOwner")</li> <li>永久焦点拥有者("focusOwner" ("activeWindow")</li> <li>默认焦点遍历策略("defa
     * ultFocusTraversalPolicy")</li> <li> </li> <li> </li> </li> </li> </li> </li> </li> </li> </li> </li> 
     * )</li> <li>当前焦点循环根目录("currentFocusCycleRoot")</li> </li>。
     * </ul>
     ...
     *
     * <p>
     *  从特定属性的侦听器列表中删除PropertyChangeListener。此方法应用于删除为特定绑定属性注册的PropertyChangeListeners。
     * <p>
     *  如果侦听器为null,则不抛出异常,并且不执行任何操作。
     * 
     * 
     ...
     *
     * <p>
     * 返回与命名属性相关联的所有<code> PropertyChangeListener </code>的数组。
     * 
     * 
     ...
     *
     * <p>
     *  触发PropertyChangeEvent以响应绑定属性中的更改。该事件将传递到所有已注册的PropertyChangeListeners。
     * 如果oldValue和newValue相同,则不会传递任何事件。
     * 
     * 
     ...
     *
     * <p>
     *  将VetoableChangeListener添加到侦听器列表。侦听器为此类的所有vetoable属性注册,包括以下内容：
     * <ul>
     *  <li>焦点所有者("focusOwner")</li> <li>永久焦点拥有者("permanentFocusOwner")</li> <li>活动窗口("activeWindow")</li>
     * </ul>
     *  如果侦听器为null,则不抛出异常,并且不执行任何操作。
     * 
     * 
     ...
     *
     * <p>
     *  从侦听器列表中删除VetoableChangeListener。此方法应用于删除为此类的所有vetoable属性注册的VetoableChangeListeners。
     * <p>
     *  如果侦听器为null,则不抛出异常,并且不执行任何操作。
     * 
     * 
     ...
     *
     * <p>
     *  返回在此键盘焦点管理器上注册的所有vetoable更改侦听器的数组。
     * 
     * 
     * @return all of this keyboard focus manager's
     ...
     *
     * <p>
     *  将VetoableChangeListener添加到特定属性的侦听器列表。指定的属性可以是用户定义的,也可以是以下之一：
     * <ul>
     *  <li>焦点所有者("focusOwner")</li> <li>永久焦点拥有者("permanentFocusOwner")</li> <li>活动窗口("activeWindow")</li>
     * </ul>
     *  如果侦听器为null,则不抛出异常,并且不执行任何操作。
     ...
     *
     * <p>
     * 从特定属性的侦听器列表中删除VetoableChangeListener。此方法应用于删除为特定绑定属性注册的VetoableChangeListeners。
     * <p>
     *  如果侦听器为null,则不抛出异常,并且不执行任何操作。
     * 
     * 
     ...
     *
     * <p>
     *  返回与命名属性相关联的所有<code> VetoableChangeListener </code>的数组。
     * 
     * 
     ...
     *
     * <p>
     *  触发PropertyChangeEvent以响应vetoable属性中的更改。该事件将传递到所有注册的VetoableChangeListeners。
     * 如果VetoableChangeListener抛出PropertyVetoException,则触发一个新事件,将所有VetoableChangeListeners都恢复为旧值,然后重新抛出该异常。
     * 如果oldValue和newValue相同,则不会传递任何事件。
     * 
     * 
     * @param propertyName the name of the property that has changed
     ...
     *
     * <p>
     *  将KeyEventDispatcher添加到此KeyboardFocusManager的分派器链。
     * 这个KeyboardFocusManager将请求每个KeyEventDispatcher在最终分派KeyEvent本身之前分派用户生成的KeyEvent。
     *  KeyEventDispatchers将按照添加的顺序通知。
     * 只要一个KeyEventDispatcher从其<code> dispatchKeyEvent </code>方法中返回<code> true </code>,通知就会停止。
     * 可以添加的KeyEventDispatcher的总数没有限制,也不能添加特定KeyEventDispatcher实例的次数。
     * <p>
     *  如果指定了空分派器,则不会执行任何操作,也不会抛出异常。
     * <p>
     * 在多线程应用程序中,{@link KeyEventDispatcher}的行为与其他AWT侦听器相同。
     * 有关详细信息,请参见<a href="doc-files/AWTThreadIssues.html#ListenersThreads"> AWT线程问题</a>。
     * 
     * 
     ...
     *
     * <p>
     *  删除以前添加到此KeyboardFocusManager的分派器链中的KeyEventDispatcher。
     * 这个KeyboardFocusManager本身不能被删除,除非它通过调用<code> addKeyEventDispatcher </code>显式地重新注册。
     * <p>
     *  如果指定了空分派器,如果指定的分派器不在分派器链中,或者如果在未显式重新注册的情况下指定此KeyboardFocusManager,则不会执行任何操作,也不会抛出异常。
     * <p>
     *  在多线程应用程序中,{@link KeyEventDispatcher}的行为与其他AWT侦听器相同。
     * 有关详细信息,请参见<a href="doc-files/AWTThreadIssues.html#ListenersThreads"> AWT线程问题</a>。
     * 
     ...
     * 除非通过调用<code> addKeyEventDispatcher </code>显式地重新注册,否则列表将不包括此KeyboardFocusManager。
     * 如果没有注册其他KeyEventDispatcher,则实现可以自由返回null或长度为0的列表。客户端代码不应该假设另一个行为,也不应该假定行为一旦建立就不会改变。
     * 
     * 
     ...
     *
     * <p>
     * 向此KeyboardFocusManager的后处理器链中添加一个KeyEventPostProcessor。
     * 在KeyEvent被分派到其目标并由其目标处理之后,KeyboardFocusManager将请求每个KeyEventPostProcessor执行任何必要的后处理作为KeyEvent的最终解决方案的一
     * 部分。
     * 向此KeyboardFocusManager的后处理器链中添加一个KeyEventPostProcessor。
     *  KeyEventPostProcessor将按照添加的顺序通知;当前的KeyboardFocusManager将被最后通知。
     * 只要一个KeyEventPostProcessor从其<code> postProcessKeyEvent </code>方法中返回<code> true </code>,通知就会停止。
     * 可以添加的KeyEventPostProcessors的总数没有限制,也不能添加特定KeyEventPostProcessor实例的次数。
     * <p>
     *  如果指定了空后处理器,则不会执行任何操作,也不会抛出任何异常。
     * <p>
     *  在多线程应用程序中,{@link KeyEventPostProcessor}的行为与其他AWT侦听器相同。
     * 有关详细信息,请参见<a href="doc-files/AWTThreadIssues.html#ListenersThreads"> AWT线程问题</a>。
     * 
     * 
     ...
     *
     * <p>
     *  从这个KeyboardFocusManager的后处理器链中删除以前添加的KeyEventPostProcessor。这个KeyboardFocusManager本身不能从链中完全删除。
     * 只能通过<code> addKeyEventPostProcessor </code>添加的附加引用可以删除。
     * <p>
     * 如果指定了空后处理器,如果指定的后处理器不在后处理器链中,或者如果未明确添加该KeyboardFocusManager,则不会执行任何操作,也不会抛出任何异常。
     * <p>
     *  在多线程应用程序中,{@link KeyEventPostProcessor}的行为与其他AWT侦听器相同。
     * 有关详细信息,请参见<a href="doc-files/AWTThreadIssues.html#ListenersThreads"> AWT线程问题</a>。
     * 
     ...
     *
     * <p>
     *  将此KeyboardFocusManager的KeyEventPostProcessor链作为列表返回。
     * 除非通过调用<code> addKeyEventPostProcessor </code>显式添加,否则列表将不包括此KeyboardFocusManager。
     * 如果没有注册KeyEventPostProcessors,则实现可以自由返回null或长度为0的列表。客户端代码不应该假设另一个行为,也不应该假设行为一旦建立就不会改变。
     * 
     * 
     ...
     * <p>
     *  请小心更改此方法！它使用反射从javax.swing.JComponent.runInputVerifier()调用。
     * 
     */
     ...
     * 期望所有KeyboardFocusManagers将分派所有FocusEvent,所有与焦点相关的WindowEvent和所有KeyEvent。
     * 这些事件应该基于KeyboardFocusManager的焦点所有者以及焦点和活动窗口的概念来调度,有时会覆盖指定的AWTEvent的源。
     * 必须使用<code> redispatchEvent </code>来执行分派,以防止AWT事件分派器递归请求KeyboardFocusManager再次分派事件。
     ...
     *
     * <p>
     *  以这种方式重新分派AWTEvent,以使AWT事件分派器不会递归地请求KeyboardFocusManager或任何已安装的KeyEventDispatchers再次分派该事件。
     * 客户端实现<code> dispatchEvent </code>和客户定义的KeyEventDispatchers必须调用<code> redispatchEvent(target,e)</code>
     * 而不是<code> target.dispatchEvent 。
     *  以这种方式重新分派AWTEvent,以使AWT事件分派器不会递归地请求KeyboardFocusManager或任何已安装的KeyEventDispatchers再次分派该事件。
     * <p>
     *  此方法旨在仅由KeyboardFocusManagers和KeyEventDispatchers使用。它不是一般客户使用。
     * 
     * 
     ...
     *
     * <p>
     * 通常,如果调度器链中没有其他KeyEventDispatcher调度KeyEvent,或者没有注册其他KeyEventDispatchers,则此方法将由<code> dispatchEvent </code>
     * 调用。
     * 如果该方法的实现返回<code> false </code>,则<code> dispatchEvent </code>可以尝试分派KeyEvent本身,或者可以简单地返回<code> false </code>
     * 。
     * 如果返回<code> true </code>,<code> dispatchEvent </code>应返回<code> true </code>。
     * 
     * 
     ...
     * 默认情况下,此方法将通过占用事件并激活快捷方式来处理映射到AWT <code> MenuShortcut </code>的任何未使用的KeyEvent。
     * 
     * 
     ...
     * 如果不是,焦点遍历将继续进行,就像focusedComponent是当前焦点所有者。
     * 
     * 
     ...
     *
     * <p>
     * 由AWT调用以通知KeyboardFocusManager它应该延迟分发KeyEvents,直到指定的组件成为焦点所有者。
     * 如果客户端代码请求焦点更改,并且AWT确定此请求可能由本地窗口系统授予,则AWT将调用此方法。
     *  KeyboardFocusManager的职责是延迟分发具有晚于指定时间戳的时间戳的KeyEvent,直到指定的组件接收到FOCUS_GAINED事件,或者AWT通过调用<code> dequeueK
     * eyEvents </code>或<code>取消延迟请求, discardKeyEvents </code>。
     * 如果客户端代码请求焦点更改,并且AWT确定此请求可能由本地窗口系统授予,则AWT将调用此方法。
     * 
     * 
     ...
     *
     * <p>
     *  由AWT调用以通知KeyboardFocusManager它应该取消延迟调度KeyEvents。
     * 由于对具有相同时间戳和组件的<code> enqueueKeyEvents </code>的调用而排队的所有KeyEvent应该被释放,以便正常分派到当前焦点所有者。
     * 如果给定时间戳小于零,则应取消具有<b>最旧的</b>时间戳(如果有)的给定组件的未完成入队请求。
     * 
     * 
     ...
     * <p>
     *  由AWT调用以通知KeyboardFocusManager它应该取消延迟调度KeyEvents。
     * 由于对相同组件的<code> enqueueKeyEvents </code>的一个或多个调用而排队的所有KeyEvent应该被丢弃。
     * 
     * 
     ...
     * 然而,如果aComponent的焦点循环根是一个窗口,那么焦点所有者通常被设置为窗口的默认组件来焦点,并且当前的焦点循环根是不变的。
     * 
     * 
     * @param aComponent the Component that is the basis for the focus
     *        traversal operation
     ...
     *
     * <p>
     *  将焦点向下移动一个焦点遍历周期。通常,如果aContainer是焦点循环根,则焦点所有者将设置为要集中的Container的默认组件,并且当前焦点循环根将设置为aContainer。
     ...
     * <p>
     *  将焦点从当前焦点所有者向上移动一个焦点遍历周期。通常,新焦点所有者设置为当前焦点所有者的焦点循环根,并且当前焦点循环根设置为新焦点所有者的焦点循环根。
     * 然而,如果当前焦点所有者的焦点循环根是窗口,则通常焦点所有者被设置为焦点循环根的默认组件以聚焦,并且当前焦点循环根不变。
     * 
     */
     ...
     * occurs.
     * <p>
     * 当且仅当当前焦点所有者是一个焦点循环根的Container时,将焦点从当前焦点所有者向下移动一个焦点遍历周期。
     * 通常,焦点所有者设置为当前焦点所有者的默认组件以进行焦点,并将当前焦点循环根设置为当前焦点所有者。如果当前焦点所有者不是作为焦点循环根的Container,则不会进行焦点遍历操作。
     * 
     */
     ...
     * newFocusOwner.
     * <p>
     *  heavyweightRequests用作currentLightweightRequests,clearingCurrentLightweightRequests和newFocusOwner的同步更
     * 改的监视器。
     * 
     */
     ...
     * <p>
     *  指示本机实现是否应该继续处理挂起的本机聚焦请求。在本地级别更改焦点之前,AWT实现应该总是调用此函数以获得权限。
     * 如果重复请求在其前面,或者指定的重量级组件已经拥有焦点,且没有原生焦点更改待处理,则此函数将拒绝该请求。
     * 否则,该请求将被批准并且焦点请求列表将被更新,使得在接收到对重量级的对应的FOCUS_GAINED事件时,如果需要,正确的子孙将被聚焦。
     * 
     * 实现必须确保对此方法和原生焦点更改的调用是原子的。如果这不能保证,则焦点请求列表的排序可能不正确,导致在类型提前机制中的错误。
     * 通常,这是通过从本地事件泵线程调用此函数,或通过在调用期间持有全局本地锁来实现的。
     * 
     */
     ...
     * <p>
     *  返回在处理此请求后将处于活动状态的窗口,如果这是重复请求,则返回null。活动窗口很有用,因为一些本机平台不支持将本机焦点所有者设置为null。
     * 在这些平台上,明显的选择是将焦点所有者设置为活动窗口的焦点代理。
     * 
     */
     ...
     * <p>
     *  用于在调度焦点事件(在focusLost / focusGained回调中)中处理异常。
     * 
     * 
        ...
        try {
            if (localLightweightRequests != null) {
                ...
                for (Iterator<KeyboardFocusManager.LightweightFocusRequest> iter = localLightweightRequests.iterator(); iter.hasNext(); ) {
                     ...
                     *  警告：这是基于DKFM的逻辑！
                     * 
                     *  我们允许在分派进程中触发restoreFocus(),只有当我们有最后一个分派请求时。如果最后一个请求失败,则焦点将恢复到上一次已成功完成请求的组件或恢复到此清除过程之前的焦点所有者。
                     * 
                     */
                    if (!iter.hasNext()) {
                     ...
                     * lw requests.
                     * <p>
                     *  当前焦点所有者为null时,我们不会调度FOCUS_LOST。但是不管它是否为null,我们都清除了所有的本地lw请求。
                     * 
                     */
                    if (currentFocusOwner != null) {
        ...
        synchronized (heavyweightRequests) {
                ...
                nativeSource == hwFocusRequest.heavyweight)
            {
                ...
                if (currentFocusOwner != null) {
                     ...
                     * <p>
                     * 因为我们收到FOCUS_GAINED当前焦点所有者不为null,相应的FOCUS_LOST应该丢失。
                     * 因此,我们保持新焦点所有者以确定合成FOCUS_LOST事件,将由KeyboardFocusManager为此FOCUS_GAINED生成。
                     * 
                     *  这段代码基于DefaultKeyboardFocusManager的实现知识,可能不适用于另一个KeyboardFocusManager。
                     * 
                     */
    ...
    static AWTEvent retargetFocusEvent(AWTEvent event) {
        ...
        synchronized(heavyweightRequests) {
             ...
             * Fix for 4472032
             * <p>
             *  此代码处理由FOCUS_GAINED的DefaultKeyboardFocusManager生成的FOCUS_LOST事件。
             * 
             *  这段代码基于DefaultKeyboardFocusManager的实现知识,可能不适用于另一个KeyboardFocusManager。
             * 
             *  修正4472032
