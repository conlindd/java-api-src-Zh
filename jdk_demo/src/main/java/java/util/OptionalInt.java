 ...
 *
 * <p>
 *  一个容器对象,可能包含或不包含{@code int}值。如果一个值存在,{@code isPresent()}将返回{@code true}和{@code getAsInt()}将返回值。
 * 
 *  <p>提供依赖于包含值是否存在的其他方法,例如{@link #orElse(int)或Else()}(如果值不存在则返回默认值)和{@link #ifPresent java.util.function.IntConsumer)ifPresent()}
 * (如果值存在,执行一个代码块)。
 * 
 *  <p>这是<a href="../lang/doc-files/ValueBased.html">以价值为基础的</a>类;在{@code OptionalInt}的实例上使用身份敏感操作(包括引用相
...
public final class OptionalInt {
     ...
     *  构造一个空实例。
     * 
     *  @implNote通常每个虚拟机只能有一个空实例{@link OptionalInt#EMPTY}。
     * 
     */
    private OptionalInt() {
     ...
     *
     * <p>
     *  返回一个空的{@code OptionalInt}实例。此OptionalInt不存在任何值。
     * 
     ...
     *
     * <p>
     *  如果此{@code OptionalInt}中存在一个值,则返回该值,否则抛出{@code NoSuchElementException}。
     * 
     * 
     ...
     *
     * <p>
     *  指示某个其他对象是否"等于"此OptionalInt。另一个对象被认为是等于：
     * <ul>
     *  <li>它也是一个{@code OptionalInt}和; <li>两个实例都没有值; <li>当前值通过{@code ==}彼此"相等"。
     * </ul>
     * 
     * 
