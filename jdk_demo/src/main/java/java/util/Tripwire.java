 ...
 *
 * <p>
 *  用于检测{@code java.util}类中无意使用拳击的实用程序类。
 * 根据{@link Boolean#getBoolean(String)},系统属性{@code org.openjdk.java.util.stream.tripwire}是否被视为{@code true}
 * ,系统会打开或关闭检测。
 *  用于检测{@code java.util}类中无意使用拳击的实用程序类。这通常应在生产使用时关闭。
 * 
 *  @apiNote典型的用法是对拳击代码做：<pre> {@ code if(Tripwire.ENABLED)Tripwire.trip(getClass(),"{0}调用PrimitiveItera
 * 
 * @since 1.8
 */
